...
void markHorizontal(ll a, ll b, bool status[10][10])
{
    ll i;
    for(i= 1; i<=8; i++) status[a][i]= 1;
}

void markVertical(ll a, ll b, bool status[10][10])
{
    ll i;
    for(i= 1; i<=8; i++) status[i][b]= 1;
}

void markDiagonal(ll a, ll b, bool status[10][10])
{
    ll i, j;
    for(i= a, j= b; i<=8 && j<=8; i++, j++) status[i][j]= 1;
    for(i= a-1, j= b-1; i>=1 && j>=1; i--, j--) status[i][j]= 1;
    for(i= a, j= b; i>=1 && j<=8; i--, j++) status[i][j]= 1;
    for(i= a+1, j= b-1; i<=8 && j>=1; i++, j--) status[i][j]= 1;
}

void init(bool a[10][10], bool b[10][10])
{
    ll i, j;
    for(i= 1; i<=8; i++)
    {
...
ll solve(ll k, bool status[10][10])
{
    if(k== 9) return 0;
    ll i;
    ll ans= neginf;
    for(i= 1; i<=8; i++)
    {
        if(status[k][i]== 0)
        {
            bool nstatus[10][10];
            init(status, nstatus);
            markHorizontal(k, i, nstatus);
            markVertical(k, i, nstatus);
            markDiagonal(k, i, nstatus);
            ll x= grid[k][i]+solve(k+1, nstatus);
            ans= max(ans, x);
        }
    }
    return ans;
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i= 1; i<=8; i++)
        {
            for(j= 1; j<=8; j++) scanf("%lld", &grid[i][j]);
        }
        bool status[10][10];
        memz(status);
        printf("%5lld\n", solve(1, status));
