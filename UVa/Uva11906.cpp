...
void goriberBFS(node s)
{
    if(dist[s]!= -2) dist[s]= 0;
    else return;
    queue<node> q;
    q.push(s);
    ll i, j;
    while(!q.empty())
    {
        ...
        for(i= 1; i<=2; i++)
        {
            for(j= 1; j<=2; j++)
            {
                ...
                ll xx= x+mm;
                ll yy= y+nn;
                if((xx>=1 && xx<=r) && (yy>=1 && yy<=c) && dist[node(xx, yy)]!= -2)
                {
                    if(dist[node(xx, yy)]== -1)
                    {
                        dist[node(xx, yy)]= dist[node(x, y)]+1;
                        q.push(node(xx, yy));
                    }
//                    else if(dist[node(xx, yy)]<(dist[node(x, y)]+1))
//                    {
//                        dist[node(xx, yy)]= dist[node(x, y)]+1;
//                        q.push(node(xx, yy));
//                    }
...
ll getCount(node s)
{
    ...
    for(i= 1; i<=2; i++)
    {
        for(j= 1; j<=2; j++)
        {
            ...
            ll xx= x+mm;
            ll yy= y+nn;
            if((xx>=1 && xx<=r) && (yy>=1 && yy<=c) && dist[node(xx, yy)]>=0)
            {
...
//        }
        ...
        for(i= 1; i<=r; i++)
        {
            for(j= 1; j<=c; j++)
            {
                if(dist[node(i, j)]>=0)
                {
                    ll cnt= getCount(node(i, j));
                    //cout<<"i,j: "<<i<<","<<j<<": cnt:"<<cnt<<endl;
                    if(cnt%2== 0) even++;
                    else odd++;
                }
            }
