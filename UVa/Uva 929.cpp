...
ll arr[1005][1005], n, m;
ll dis[1005][1005];
struct node
{
    ll r, c, d;
    node(ll x, ll y, ll dst)
    {
        r= x;
        c= y;
        d= dst;
...
ll dijkstra()
{
    ...
    while(!pq.empty())
    {
        ...
        j= u.c;
        if(i== n && j== m) break;
        if(j+1<=m && (dis[i][j]+arr[i][j+1])<dis[i][j+1])
        {
            dis[i][j+1]= dis[i][j]+ arr[i][j+1];
            pq.push(node(i, j+1, dis[i][j+1]));
        }
        if(j-1>=1 && (dis[i][j]+arr[i][j-1])<dis[i][j-1])
        {
            dis[i][j-1]= dis[i][j]+ arr[i][j-1];
            pq.push(node(i, j-1, dis[i][j-1]));
        }
        if(i+1<=n && (dis[i][j]+arr[i+1][j])<dis[i+1][j])
        {
            dis[i+1][j]= dis[i][j]+ arr[i+1][j];
            pq.push(node(i+1, j, dis[i+1][j]));
        }
        if(i-1>=1 && (dis[i][j]+arr[i-1][j])<dis[i-1][j])
        {
            dis[i-1][j]= dis[i][j]+ arr[i-1][j];
            pq.push(node(i-1, j, dis[i-1][j]));
        }
