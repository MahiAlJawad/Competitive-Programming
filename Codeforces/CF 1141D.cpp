...
ll avla[125], avlb[125];
vector<ll> a[125], b[125];
struct node
{
    ll x, y;
};
...
int main()
{
    ...
    for(i= 63; i<=122; i++) avla[i]= a[i].size();
    for(i= 63; i<=122; i++) avlb[i]= b[i].size();

    for(i= 97; i<=122; i++)
    {
        if(avla[i]== 0 && avlb[i]== 0) continue;
        if(avla[i]>=avlb[i])
        {
            for(j= avlb[i]-1; j>=0; j--)
            {
                node nd;
                nd.y= b[i][j];
                nd.x= a[i][avla[i]-1];
            ...
            if(avla[i]== 0 && avlb[63]== 0) continue;
            if(avla[i]>= avlb[63])
            {
                for(j= avlb[63]-1; j>=0; j--)
                {
                    node nd;
                    nd.y= b[63][j];
                    nd.x= a[i][avla[i]-1];
        ...
        else// a is less
        {
            ...
            avla[i]= 0;
            if(avlb[i]== 0 && avla[63]== 0) continue;
            if(avlb[i]>= avla[63])
            {
                for(j= avla[63]-1; j>=0; j--)
                {
                    node nd;
                    nd.x= a[63][j];
                    nd.y= b[i][avlb[i]-1];
