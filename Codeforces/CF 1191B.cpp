...
void fun(string s)
{
    if(s[1]== 'm') v[1].pb((int)(s[0]-48));
    else if(s[1]== 'p') v[2].pb((int)(s[0]-48));
    else v[3].pb((int)(s[0]-48));
}
int main()
{
    //fasterInOut;
    ...
    fun(s3);
    int i, j;
    for(i= 1; i<=3; i++) sort(v[i].begin(), v[i].end());

    int y= 100000000;
    for(i= 1; i<=3; i++)
    {
        if((int)v[i].size()== 1) y= min(2, y);
        else if((int)v[i].size()== 2)
        {
            int d= (int)v[i][1]- (int)v[i][0];
            if(d== 1) y= min(1, y);
            else if(d==2) y= min(1, y);
            else y= min(2, y);
        }
        else if((int)v[i].size()== 3)
        {
            int d1= (int)v[i][1]- (int)v[i][0];
            int d2= (int)v[i][2]- (int)v[i][1];
            if(d1== 1 && d2== 1) y= min(0, y);
            else if(d1== 1 || d2== 1 || d1== 2 || d2== 2) y= min(1, y);
            else y= min(2, y);
