...
ll indexdis(ll x, ll y)
{
    return (max(x, y)-min(x, y))-1;
}
ll lp[200005], rp[200005];
int main()
{
//    fasterInOut;
    string s, t;
    ...
    for(i= 0; i<s2; i++)
    {
        if(i== 0)
        {
            ll m= indexdis(lp[i], -1);
            ll n= indexdis(rp[i], -1);
            mx= max(mx, max(m, n));
        }
        else
        {
            ll a= indexdis(lp[i], lp[i-1]);
            ll b= indexdis(rp[i], rp[i-1]);
            ll c= indexdis(rp[i], lp[i-1]);
            ll d= indexdis(lp[i], rp[i-1]);
            mx= max(mx, max(a, max(b, max(c, d))));
        }
    }

    mx= max(mx, max(indexdis(s1, lp[s2-1]), indexdis(s1, rp[s2-1])));
    cout<<mx<<"\n";
