...
ll c[5005], a[5005], sz[5], mark[5005];
int main()
{
    ...
    for(i= 1; i<=n;i++)
    {
        if(c[i]== 0 && a[i]== 0) v[1].pb(i);
        else if(c[i]== 0 && a[i]== 1) v[2].pb(i);
        if(c[i]== 1 && a[i]== 0) v[3].pb(i);
        if(c[i]== 1 && a[i]== 1) v[4].pb(i);
    }

    for(i= 1; i<=4; i++) sz[i]= v[i].size();
    vector<ll> ans;
    if(sz[2]== sz[3] && sz[1]%2== 0 && sz[4]%2== 0)
    {
        for(i= 0;  i<sz[2]; i++) ans.pb(v[2][i]);
        for(i= 0; i<sz[1]/2; i++) ans.pb(v[1][i]);
        for(i=0; i<sz[4]/2; i++) ans.pb(v[4][i]);
