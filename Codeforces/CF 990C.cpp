...
int main()
{
    ...
    for(i= 1; i<=n; i++)
    {
        ...
        r[i]= pos;
        l[i]= neg;
        //cout<< "i: "<<i<< " pos: "<<pos<< " neg: " <<neg<<endl;
        if(pos>0 && neg>0) continue;
        else if(neg>0 && pos== 0) mp[(-1)*neg]++;
        else if(neg== 0 && pos== 0) mp[0]++;
    }
    ...
    for(i= 1; i<=n; i++)
    {
        ...
        ll neg= l[i];
        if(pos>0 && neg>0) continue;
        if(pos== 0 && neg== 0) cnt+= mp[0];
        else if(pos>0) cnt+= mp[(-1)*pos];
    }
