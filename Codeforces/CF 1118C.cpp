...
void printarray(ll arr[25][25], ll n)
{
    ll i, j;
    for(i= 1; i<=n; i++)
    {
...
int main()
{
    ...
    if((n%2== 1 && nmofOdd!= 1) || (n%2== 0 && nmofOdd!= 0))
    {
        cout<<"NO\n";
        return 0;
    }
    ...
    if(n%2== 1)
    {
        ...
        for(i= 0;i<v.size(); i++)
        {
            ...
            if(nd.y%2== 1)
            {
                d= nd.x;
                v[i].y--;
                if(v[i].y== 0) vectorEraseSingle(v, i);
                v[i].z= v[i].y%4;
                break;
            }
        }
        ...
        for(i= 1, j= n; i<mid; i++, j--)
        {
            for(k= 1; k<mid; k++)
            {
                if(param>= v.size() || v[param].y%4!= 0)
                {
                    cout<<"NO\n";
                    return 0;
                }
                //cout<<"param: "<<v[param].x<<" "<<v[param].y<<endl;
                ll num= v[param].x;
                v[param].y-= 4;
                if(v[param].y== 0) param++;
