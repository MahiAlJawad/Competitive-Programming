...
ll coin_change(ll i, ll amt)
{
    ...
    if(limit[1]>0)
    {
        x= ((coin_change(i+1, amt)%mod)*(limit[1]%mod))%mod;
//        for(j= 1; j<=limit[1]; j++)
//        {
//            x+= coin_change(i+1, amt)%mod;
//        }
    }
    if(limit[2]>0)
    {
        y= ((coin_change(i+1, amt+1)%mod)*(limit[2]%mod))%mod;
//        for(j= 1; j<=limit[2]; j++)
//        {
//            y+= coin_change(i+1, amt+1)%mod;
//        }
    }
    if(limit[3]>0)
    {
        z= ((coin_change(i+1, amt+2)%mod)*(limit[3]%mod))%mod;
//        for(j= 1; j<=limit[3]; j++)
//        {
//            z+= coin_change(i+1, amt+2)%mod;
//        }
    }

    return dp[i][amt]= (x+y+z)%mod;
}
...
int main()
{
    ...
    limit[1]= floor((double)r/3.0)- floor((double)(l-1)/3.0);
    limit[2]= floor((double)(r-1)/3.0)- floor((double)(l-2)/3.0);
    limit[3]= floor((double)(r-2)/3.0)- floor((double)(l-3)/3.0);
    memneg(dp);
    //cout<<limit[1]<<" "<<limit[2]<<" "<<limit[3]<<endl;
    cout<<coin_change(1, 0)<<"\n";
