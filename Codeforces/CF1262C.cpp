...
ll a[105], k, pos, dp[105][105], dir[105][105], n;
...
ll solve(ll i, ll left)
{
    ...
    ll x, y;
    if(left-1>=0) x= a[i]+solve(i+1, left-1);
    else x= 0;
    y= solve(i+1, left);
...
vector<ll> getDir(ll i, ll left, vector<ll> v)
{
    if(i== n+1)
    {
        return v;
    ...
    {
        v.pb(a[i]);
        return getDir(i+1, left-1, v);
    }
    else if(dir[i][left]== 2)
    {
        return getDir(i+1, left, v);
    }
    else if(dir[i][left]== 3)
    {
        vector<ll> t1, t2;

        t1= getDir(i+1, left, v);
        v.pb(a[i]);
        t2= getDir(i+1, left-1, v);
