...
void printst(ll n, bool status[100005])
{
    ll i;
    for(i= 1; i<=n;  i++) cout<<status[i]<<" ";
    cout<<endl;
}
ll solve(ll i, ll n, bool status[100005])
{
    if(i>= n+1 || i<=0 || status[i]== 1) return 0;
    ...
    if(arr[i]== 1)
    {
        x= solve(i+1, n, status);
        y= solve(i-1, n, status);
    }
    else
    {
        x= solve(i+1, n, status);
        y= solve(i-1, n, status);
        p= solve(i+2, n, status);
        q= solve(i-2, n, status);
    }
    //cout<<"after: ";
    //cout<<s<<endl;
    //printst(n, status);
