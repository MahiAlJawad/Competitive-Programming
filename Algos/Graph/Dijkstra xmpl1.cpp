...
ll dijkstra(vector<ll> adj[105], ll src)
{
...
int main()
{
    ...
    //fin(in);
    //fout(out);
    scanf("%lld", &tc);//test case input

    for(i= 1; i<=tc; i++)
    {
        if(i!= 1) printf("\n");

        scanf("%lld%lld%lld", &n, &e, &t);//number of node, ending node, countdown timer
        vector<ll> adj[105];
        ll m;
        scanf("%lld", &m);//number of edges
        memneg(cost);//initializing with inf= -1 value
        while(m--)
        {
            ll a, b, w;
        ...
        for(j= 1; j<=n; j++)
        {
            //every j is a different source node for the graph
            memneg(dis);
            //cout<<"j: "<<j<<" ";
            ll dis= dijkstra(adj, j);
            //cout<<"dis: "<<dis<<endl;
            if(dis<= t && dis>=0) cnt++;
        }
